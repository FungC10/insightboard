      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Real-time cryptocurrency market data, advanced analytics, and professional-grade insights
          </p>
        </div>
                ...
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center">
                    <div className="w-14 h-14 bg-gradient-to-br from-orange-400 to-orange-600 rounded-2xl flex items-center justify-center mr-4 shadow-lg">
                      <span className="text-orange-100 font-bold text-xl">₿</span>
                    </div>
                    <div>
                  ...
                  <Badge variant={bitcoin.priceChange24h >= 0 ? 'success' : 'danger'} className="text-lg px-4 py-2">
                    {bitcoin.priceChange24h >= 0 ? '+' : ''}{bitcoin.priceChange24h.toFixed(2)}%
                  </Badge>
                </div>
              ...
              <Card className="p-8 bg-white/80 backdrop-blur-sm shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center">
                    ...
                    <div>
                      <h3 className="text-2xl font-bold text-gray-900">Ethereum</h3>
                      <p className="text-gray-500 font-medium">ETH</p>
                    </div>
                  </div>
                  <Badge variant={ethereum.priceChange24h >= 0 ? 'success' : 'danger'} className="text-lg px-4 py-2">
                    {ethereum.priceChange24h >= 0 ? '+' : ''}{ethereum.priceChange24h.toFixed(2)}%
                  </Badge>
                </div>
                <div className="text-4xl font-bold text-gray-900 mb-6">
        ...
        <Card className="p-8 bg-white/80 backdrop-blur-sm shadow-xl">
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-3xl font-bold text-gray-900">Market Overview</h3>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b-2 border-gray-200">
                  <th className="text-left py-4 px-6 font-bold text-gray-700 text-lg">Coin</th>
                  <th className="text-right py-4 px-6 font-bold text-gray-700 text-lg">Price</th>
                  <th className="text-right py-4 px-6 font-bold text-gray-700 text-lg">24h Change</th>
                  <th className="text-right py-4 px-6 font-bold text-gray-700 text-lg">Market Cap</th>
                  <th className="text-right py-4 px-6 font-bold text-gray-700 text-lg">Volume</th>
                </tr>
              </thead>
              <tbody>
                {coins?.map((coin, index) => (
                  <tr key={coin.id} className="border-b border-gray-100 hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 transition-all duration-200">
                    <td className="py-6 px-6">
                      <div className="flex items-center">
                        ...
                        <div>
                          <div className="font-bold text-gray-900 text-lg">{coin.name}</div>
                          <div className="text-gray-500 font-medium">{coin.symbol.toUpperCase()}</div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="text-right py-6 px-6">
                      <Badge variant={coin.priceChange24h >= 0 ? 'success' : 'danger'} className="text-base px-4 py-2">
